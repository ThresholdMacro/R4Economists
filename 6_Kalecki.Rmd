...
## - input data here 
Kaleckidata <- readr::read_csv("Data/Kaleckidata.csv")
KaleckiPctGDP <- readr::read_csv("Data/KaleckiPctGDP.csv")
colnames(KaleckiPctGDP) <- gsub("\\."," ", names(KaleckiPctGDP))

K_identities <- names(KaleckiPctGDP[,-1])

selectizeInput(
    inputId = "Kalecki_ids",
    label= "Choose identities to visualize",
    choices = K_identities,
    selected = c(
      "Net Govt Saving",
      "Foreign Saving",
      "Net Investment",
...
selectizeInput(
    inputId = "cor",
    label= corrtxt,
    choices = c(K_identities),
    selected = c(K_identities[c(7,6)]),
    multiple = TRUE,
...
selectizeInput(
    ...
    choices = c(K_identities),
    selected = c(K_identities[c(6,2,3)]),
    multiple = TRUE,
...
dataSelected <- reactive({
 ...
 alldata <-
   Kaleckidata |>
   select(Date, all_of(input$Kalecki_ids)) |>
...
# # # any further data wrangling
    ...
    titletxt <- "Selected Kalecki identities: USD bln"
    yaxistitle <- "USD blns"
...
# plot the result 
  hchartHAhist(cordf,ctitle,upperCI)
...
# # # any further data wrangling
    ...
    lngdata <- lngdf(alldata)
    citation <- 'Source: Bureau of Economic Analysis, FRED'
    titletxt <- "Selected Kalecki identities: % of GDP"
