...
4.  Session 4: Build a (slightly) interactive dashboard - Look at more complex dashboards

## Session 1

Setting Up the R Environment, basic calculation, data

-   You should already have: - Setup a Posit cloud account to create an on-line RStudio environment <https://posit.cloud/> . If you need help, this is very useful: <https://learnr-examples.shinyapps.io/ex-setup-r/> (available in my Git repo).
...
-   One big advantage of R is in its ability to work with vectors.
    ...
    -   Raise each element of a vector to a power (or other manipulation)
...
## Simple calcs: assign values, vectors

-   The return from an interest rate over 2 periods. i =0.05 and t =2 .
    -   accum holds the result of the calculation of 1+𝑖\*𝑡 for the values of i 𝑖 and t 𝑡.
    -   Exponentiate the quantity (1+𝑖) to the 𝑡 th power.
    -   Create a sequence for t & recalculate.
    -   Create a dataframe.
...
5.  [factor]{.underline} ( used to represent categorical data. Factors can be ordered or unordered and are an important class for statistical analysis and for plotting). All data structures have dimensions, names, types, contents.
...
-   d b.nomics.world is a great source of global economic data from numerous providers, delivered in a standard (or nearly standard) format. This makes it very useful.
...
-   Most examples taken from Dr Nick Huntington-Klein: [https://www.youtube.com/\@NickHuntingtonKlein](https://www.youtube.com/)
-   Linear models, fancy linear models, some tests for robust errors
-   Time series
-   ARIMA
-   Use script Sess3_1RegModelsTS_ARIMA.R in R directory
...
-   Use Rmarkdown file Sess4_Kalecki
